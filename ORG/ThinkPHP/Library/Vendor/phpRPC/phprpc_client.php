...
/*
 * PHPRPC Client for PHP. Copyright: Ma Bingyao <andot@ujn.edu.cn> Version: 3.0 LastModified: Apr 12, 2010 This library is free. You can redistribute it and/or modify it under GPL. /* Interfaces $rpc_client = new PHPRPC_Client(); $rpc_client->setProxy(NULL); $rpc_client->useService('http://www.phprpc.org/server.php'); $rpc_client->setKeyLength(1024); $rpc_client->setEncryptMode(3); $args = array(1, 2); echo $rpc_client->invoke('add', &$args); echo "<br />"; $n = 3; $args = array(&$n); echo $rpc_client->invoke('inc', &$args, true); echo "<br />"; echo $rpc_client->sub(3, 2); echo "<br />"; // error handle $result = $rpc_client->mul(1, 2); // no mul function if (is_a($result, "PHPRPC_Error")) { echo $result->toString(); }
 */
